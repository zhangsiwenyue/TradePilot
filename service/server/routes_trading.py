...
def register_trading_routes(app: FastAPI, ctx: RouteContext) -> None:
    ...
    async def follow_provider(data: FollowRequest, authorization: str = Header(None)):
        ...
        # Validate copy settings. Ratio clamped to a sane band so a typo can't
        # bankrupt the follower or create dust positions; auto_copy is a flag.
        ...
        if existing:
            # Re-following = updating the copy settings.
            cursor.execute(
                """
